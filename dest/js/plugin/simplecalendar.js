    ...
    var mon = 'Mon';
		...
		function printDateNumber(monthNumber, mon, tue, wed, thur, fri, sat, sund) {
			...
			$.each(monthDays, function(index, item){ 
				$('tbody.event-calendar tr').eq(Math.floor(index / 7)).append('<td date-month="' + item.month + '" date-day="' + item.date + '" date-year="' + item.year + '"' +(item.show ? '': ' class="hidden-date"') + '><span>' + item.date  + '</span></td>');
			})
